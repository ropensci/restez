...
Now the download has begun, we just need to wait for it to finish. For the largest domains, this can take quite a while and may be prone to server errors. In this case, you may want to set the `max_tries` argument of `db_download()` to a relatively high number to automatically re-try downloading until that number of tries is exceeded (in this case, be sure to set `overwrite` to `TRUE` or the download will start over from scratch).
