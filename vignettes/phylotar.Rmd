...
We will run a `phylotaR` run for the rodent genus, *Cavia*, the clade that contains guinea pigs. Because this clade is so small, it should not take too long to run it. We will, however, limit to showcasing only the download stage of the `phylotaR` pipeline, where `restez` is required.
...
devtools::install_github(repo = 'ropensci/phylotaR', ref = 'restez')
```

##Setup
Since we will be running the `phylotaR` pipeline for *Cavia*, we can use the rodents database we created [before](https://antonellilab.github.io/restez/articles/rodents.html). We do not need to set-up the `phylotaR` pipeline any differently with a `restez` database, except to ensure we have set the `restez` path to the rodent database. (Connections to the `restez` database will be made automatically, `restez_connect` or `restez_disconnect` do not need to be run.)
...
Sequences that cannot be found locally will be downloaded via Entrez. Sequences may not be found locally for three sets of reasons, 1. phylotaR may have identified non-GenBank sequences (e.g. RefSeq), 2. the local database may have size limits (ours has a min of 100 and a max 1000), and 3. the local database may not represent all of the sequences of a taxonomic group, e.g. we have downloaded just the sequences in the Rodentia domain but rodent DNA may exist in others.

> Note: If the `phylotaR` messages do not explicit state that they are using a `restez` database, then, you may not have set `restez` path.

Compared to running without a `restez` database, phylotaR download can run [**2x** faster](https://github.com/AntonelliLab/restez/blob/master/other/phylotar_demo.R).

##Next up

**[Tips and tricks](https://antonellilab.github.io/restez/articles/tips_and_tricks.html)**