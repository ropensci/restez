---
title: "Demonstration 1: search and fetch"
author: "Dom Bennett"
date: "`r Sys.Date()`"
...
First, we need to download and setup the restez database. When we use `download_genbank()` we are going to select 'rodents'. We are downloading
...
To fetch the sequences from the database we set up, we can just use the `get_fasta` function.
